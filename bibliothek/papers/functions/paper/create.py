...
def from_dict(paper_dict, files=[]):
    ...
    for (i, author), has_next in lookahead(enumerate(paper_dict['authors'])):
        if i == 0:
            stdout.p(['Authors:', author], positions=positions, after=None if has_next else '_')
        else:
            stdout.p(['', author], positions=positions, after=None if has_next else '_')
    ...
    if paper_dict['url']:
        stdout.p(['URL:', paper_dict['url']], positions=positions)
    ...
    for (i, file), has_next in lookahead(enumerate(files)):
        stdout.p(['File:' if i == 0 else '', file], positions=positions, after=None if has_next else '_')
